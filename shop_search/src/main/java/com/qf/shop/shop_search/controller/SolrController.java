...
public class SolrController {
    ...
    /**
     * 添加索引库,添加商品的同时同步到索引库，提高查询效率，减少服务器压力
     * @RequestBody:将请求体的内容转成对象，请求体的内容是json
     * @return
     */
    @RequestMapping("/add")
    @ResponseBody
    public boolean solrAdd(@RequestBody Goods goods){
        ...
        //获取新添加的商品信息
        solrInputDocument.addField("id", goods.getId());//商品ID
        solrInputDocument.addField("gtitle", goods.getTitle());//商品标题
        solrInputDocument.addField("ginfo", goods.getGinfo());//商品介绍
        solrInputDocument.addField("gprice", goods.getPrice());//商品价格
        solrInputDocument.addField("gimage", goods.getGimage());//商品图片
